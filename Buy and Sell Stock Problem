...
NOTE: I have used a common approach to solve few problems, you can do other also same.
...
class Solution {
public:
    int find(int buy,vector<int>& prices, int i,vector<vector<int>> &v)
    {
        ...
        if(buy)
        {
           int a= -prices[i]+find(!buy,prices,i+1,v); 
           int b=  find(buy,prices,i+1,v);
            
           return v[i][buy]=max(a,b); 
        }
        else
        {
           int a=  prices[i]+find(!buy,prices,i+2,v); 
           int b=  find(buy,prices,i+1,v); 
           return v[i][buy]=max(a,b); 
        }
    }
...
class Solution {
public:
    int find(int buy,vector<int>& prices, int fee, int i,vector<vector<int>> &v)
    {
        if(i>=prices.size()) return 0;
        ...
        if(buy)
        {
           int a= -prices[i]+find(!buy,prices,fee,i+1,v); 
           int b=  find(buy,prices,fee,i+1,v);
            
           return v[i][buy]=max(a,b); 
        }
        else
        {
           int a=  prices[i]+find(!buy,prices,fee,i+1,v)-fee; 
           int b=  find(buy,prices,fee,i+1,v); 
           return v[i][buy]=max(a,b); 
        }
    }
...
**Hit ✨ if you like it :) **
**Please improve and create pull request if any improvement needed**
